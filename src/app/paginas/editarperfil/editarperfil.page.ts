...
export class EditarperfilPage implements OnInit {
  ...
  editarUsuario(form: NgForm) {
    let obj = {
      ...
      intentos: 0,
      clasesRestantes: this.ep.clasesRestantes,
    };
  ...
  eliminarFaltas(idUsuario, nombre) {
  ...
  verPaquetes(idUsuario,) {
    this.navCtrl.navigateRoot("crearpaquete/" + idUsuario + "/" + this.sociedad + "/" + this.idRol);
  ...
  async presentAlertConfirm(idUsuario, nombre) {
  ...
  faltasEliminar(idUsuario) {
    this.servicio.eliminarFaltas(idUsuario, this.idUser).subscribe((response: any) => {
      this.mensaje = response.descripcion;
  ...
  ajustarFalta(idUsuario, tipo) {
    ...
    if (p === 0) {
      this.servicio.ajustarFalta(idUsuario, this.idUser, tipo).subscribe((response: any) => {
        this.mensaje = response.descripcion;
        if (response.codigo == 500) {
          this.deleteClase();
          console.log("ID de usuario sin faltas", idUsuario);
